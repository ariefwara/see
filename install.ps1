# see — Windows installer
# Run:  powershell -ExecutionPolicy Bypass -c "irm https://raw.githubusercontent.com/ariefwara/see/main/install.ps1 | iex"
...
$TmpDir = Join-Path $env:TEMP "see-install-$([System.IO.Path]::GetRandomFileName())"
...
try {
    ...
    try {
        ...
        if ($Asset) {
            ...
            $ZipPath = Join-Path $TmpDir "see.exe"
            ...
            if (Test-Path $ZipPath) {
                ...
                Write-Info "Run 'see <image>' to preview an image"
                exit 0
            }
        }
    }
